...
set_legend = False
TrainMode = False
resume = '{}/best_model.pth'.format(save_fig_path)
...
def save_checkpoint(state, is_best, checkpoint='three_body_checkpoint', filename='checkpoint.pth.tar'):
...
if resume is not None:
    ...
    # for k, v in func.named_parameters():
    #     print(k, v)
...
def exprot_excel(data_planet, bench, index, file_name):
    e_name = './our_planet_' + str(file_name)+ '.xls'
    o_name = './our_planet_'+str(file_name) + '_bench.xls'
    wl_e = xlwt.Workbook(e_name)
    ws_e = wl_e.add_sheet('test')
    wl_o = xlwt.Workbook(o_name)
    ws_o = wl_o.add_sheet('test')
    
    row, col = 0, 0
    shape = data_planet.shape
    for i in range(shape[0]):
        ws_e.write(row, col, float(data_planet[i, 0]))
        ws_e.write(row, col + 1, float(data_planet[i, 1]))
        ws_e.write(row, col + 2, float(data_planet[i, 2]))
        ws_o.write(row, col, float(bench[0, i, index[0]]))
        ws_o.write(row, col + 1, float(bench[0, i, index[1]]))
        ws_o.write(row, col + 2, float(bench[0, i, index[2]]))
        row += 1
        col = 0
...
def test(func, save_fig_path):
    ...
    print('Three_Body_Inference_Time', end - start)
    ...
    ## export Excel
    exprot_excel(r1_sol, out_all_data, [0, 1, 2], 1)
    exprot_excel(r2_sol, out_all_data, [3, 4, 5], 2)
    exprot_excel(r3_sol, out_all_data, [6, 7, 8], 3)
    ...
    #                  plot into animation                            #
    ###################################################################
    # generate_animation_figures(out_all_data[0, :, :3], out_all_data[0, :, 3:6], out_all_data[0, :, 6:9], r1_sol, r2_sol, r3_sol, np.array(t_list), animation_fig_path)
