'''
Author: CSuperlei
Date: 2021-12-17 16:58:55
LastEditTime: 2022-01-29 11:49:41
Description: 定义前向传播和反向传播 
实现了 Implementation of ICLR 2021 paper "MALI: a memory efficient asynchronous leapfrog integrator for Neural ODEs"
'''
import torch
...
def flatten_params(params):
    '''
    description: 将func的所有参数都拉平然后cat最后成为一个列向量
    param {*} params
    return {*}
    '''
...
class Checkpointing_Adjoint(torch.autograd.Function):
    ...
    def forward(ctx, *args): # z0 = [y0, v0] for symplectic solvers
        ...
        with torch.no_grad():
            hyperparams = extract_keys(options)  ## hyperparams 是一个数组，包括options的key和value
    ...
    def backward(ctx, *grad_output):
        ...
        ### 计算BasicBlock2的输出值, solver.func 就是BasicBlock2被包装的FuncWrapper的对象
        func_i = solver.func(solver.t1, y_current[:len(y_current)//2])  ## 求出来值
        dLdt1 = sum(
...
def odesolve_adjoint_sym12(func, y0, options = None):
    '''
    description: 传入的Pytorch层 BasicBlock2
    param {*} func
    param {*} y0 : x
    param {*} options
    return {*}
    ...
    ### func是BasicBlock2的对象，y0是网络输入的值，options是积分的其实步骤
    ### 经过check_arguments, _tensor_input是一个bool型变量, _func是返回tuple的func, _y0是tuple
    _tensor_input, _func, _y0 = check_arguments(func, y0, options['t0'])

    if options['method'].lower() in ['sym12async', 'fixedstep_sym12async']:
        ### 将y0 和 y1 值合成一个tuple 赋值给 initial_condition
    ...
    # print(_y0[0].shape)  [256, 64, 32, 32]
    # print(len(_y0[0]))   256
    out = zs[0:len(_y0)]
    if _tensor_input:
        out = out[0]
