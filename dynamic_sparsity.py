...
class layerSparsity(object):
    def __init__(self, optimizer, pruning_type, sparsify_first_layer, sparsify_last_layer, inference=False):
        '''
            yaml_pruning_type=="unstructured":
        ...
        self.layers_density = {}  ## 存储动态sparsity value
            
    def get_sparsity(self):
        return self.sparsity

    def add_module(self, module, density, mode='constant', density_array=None):
        '''
        description: 
        param {*} self
        param {*} module: net
        param {*} density: 0.5 0.4 0.3 0.1
        param {*} mode
        return {*}
        '''
        self.modules.append(module)  ## modules是加入模型
        ...
        ### 将不做sparsity的BN层和bias都从self.layers移除
        ...
        for name, tensor in module.named_parameters():
            ...
            self.layers[name]=(tensor.shape, layer_num)  ## self.layers[name] = (shape, num) 元组
            if density_array is not None:
                self.layers_density[name] = density_array[name] 
            ## np.prod数组元素乘积，统计总共参数量
    ...
    def sparsity_regrowth(self):
        ### 训练过程中动态计算一个sparsity这个值，然后用self.sparisity[name]存储这个值，在训练完从字典中取这个值去做sparsity
        num_overgrowth = 0
        total_overgrowth = 0
        residual = 0
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    if (not self.sparsify_first_layer) and self.layers[name][1]==0:
                        ...
                        # sparsity=1 - density
                        if not self.layers_density:
                            sparsity = 1 - density
                        else:
                            sparsity = 1 - self.layers_density[name] 
                    self.layer_zeros[name] = weight.numel() - self.layer_non_zeros[name]  ## 记录了变为0的参数名字和参数个数
                    
                    self.sparsity[self.layers[name][1]]=(name, sparsity)  ## 记录了每层的名字以及该层的sparsity的值
