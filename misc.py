...
def norm(x):
    ...
    if torch.is_tensor(x):
        ...
        # t_s = time.time()
        s = torch.sqrt(sum(x_.norm()**2 for x_ in x) / sum(x_.numel() for x_ in x))
        # t_e = time.time()
        # print('origin_norm', t_e - t_s)
        # s = torch.abs(sum(x_.abs().sum() for x_ in x) / sum(x_.numel() for x_ in x))

        t_s = time.time()
        s = max([x_.abs().max() for x_ in x])
        t_e = time.time()
        # print('max_norm', t_e - t_s)
        return s
