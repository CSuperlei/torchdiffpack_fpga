...
def matrix_drop_th(tensor,select_percentage):
        ...
        topk            =   tensor.view(-1).abs().topk(k) ## top(k)默认是对最后一维度取前k大的值以及对应的index  value, index = tensor.top(k)
        threshold       =   topk[0][-1]
...
def drop_hw(tensor,select_percentage):
        ...
        tensor          =   tensor.reshape(tensor_shape[0],tensor_shape[1],tensor_shape[2]*tensor_shape[3])
        new_tensor_shape=   tensor.shape
        topk            =   tensor.abs().topk(k)  ## top(k)默认是对最后一维度取前k大的值对应的index
        interleaved     =   topk[0][:,:,-1].repeat_interleave(tensor_shape[2]*tensor_shape[3])  ## 默认将所有向量拉平，讲第k个值复制h*w份,然后变成一维
        interleaved     =   interleaved.reshape(new_tensor_shape)
        index           =   tensor.abs()>=(interleaved)
        index           =   index.type_as(tensor)
        tensor          =   (tensor*index)
