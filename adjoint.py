'''
Author: CSuperlei
Date: 2022-01-29 11:35:40
LastEditTime: 2022-02-16 10:50:13
Description:  实现了Implementation of ICML 2020 paper "Adaptive checkpoint adjoint method for accurate gradient esitmation in Neural ODEs"
'''
import torch
...
class Checkpointing_Adjoint(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        with torch.no_grad():
            solver = odesolve_endtime(func, z0, options, return_solver=True, regenerate_graph = False)  ## 注意 return_solver=True
            ans, steps = solver.integrate(z0, t0 = options['t0'], return_steps=True)  ## 注意 return_steps=True
